...
def find_last_year(path, var, runid="fesom"):
    ...
    if not files:
        raise FileNotFoundError(f"There are (yet) no files with variable {var}")
    files.sort()
...
def fcheck():
    parser = argparse.ArgumentParser(
        prog="fcheck", description="Check FESOM2 experiment data."
    )
    ...
    for variable in ds.index:
        ...
        print(
            f"Variable: {variable}, current_value: {current_value}, master_value: {master_value}"
        )
        try:
            assert current_value == approx(master_value, abs=args.abs)
        except:
            print(f"FAIL!!!: {variable} DIFFERENCE: {current_value-master_value}.\n")
            valid = False
    if not valid:
